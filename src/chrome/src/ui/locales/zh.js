...
export default {
  ...
  "st.display.help_improve.desc_html": "允许保留选定的 WebBrain Compass 交互，并将其用于评估、改进、微调和训练。此选项默认开启。关闭后，未来的 Compass 交互将不会用于这些用途。<u>Local-model and bring-your-own API requests are only collected by WebBrain from providers where you turn on “Share queries for research”.</u><a href=\"https://webbrain.one/privacy\" target=\"_blank\" rel=\"noopener noreferrer\" style=\"color:var(--accent);\">隐私政策 →</a>",
  "st.providers.webbrain_data_use.body": "WebBrain Compass 包含每日免费用量。帮助改进 WebBrain 默认开启时，选定的 Compass 对话可能会被保留，并用于评估、改进、微调和训练。可在“常规 → 高级”中关闭，以排除未来的 Compass 交互。<u>Local-model and bring-your-own API requests are only collected by WebBrain when you turn on the per-provider “Share queries for research” option.</u>{privacyLink}。如需更多用量，请在 {subscribeLink} 订阅。在 {accountLink} 管理账单。",
